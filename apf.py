"""
apf.py -- APF's rhythm: findings assigned over rounds, shared at a checkpoint.

THIS MODULE OWNS THE CADENCE, NOT THE CASEFILES. `casefiles.py` decides WHICH findings
land in whose casefile and proves the result winnable; this decides WHEN each of
...
Assigned once, every player makes exactly one share decision all game, and one
...
2. A SHARED FINDING STAYS IN ITS HOLDER'S CASEFILE. Owner: "in the metaphor of the
...
3. THE REQUIREMENT IS CUMULATIVE -- shared >= required(total held), not
   shared-this-round >= required(assigned-this-round). Without that, round 4 would
   demand three new findings when only one was assigned.

AND THAT REVIVES THE DIFFICULTY LADDER. Session 38 measured EASY/MEDIUM/HARD
all resolving to "share 2, keep 1" at a fixed three-finding casefile -- a percentage
...
`casefiles.solves()` counts it: item 27 is explicit that a narrowing must never
...
import casefiles as _cf

# The first round at which a share checkpoint runs. See rule 1 above: at a casefile
...
def round_count(pool_size: int, player_count: int) -> int:
    """Rounds = findings / players, floored. No finding is assigned twice, so the
...
def casefile_spec_for(rounds: int) -> tuple:
    """A casefile spec whose LENGTH is the round count.

    `casefiles.py` already expresses a casefile as one slot per kind, and under the
    rhythm that casefile is assigned one slot per round -- so the round count needs no
    new constraint code, only a spec of the right length. Extra slots take
    `casefiles._FALLBACK_KIND` (evidence), which is the largest pool in every mystery
    on disk and already the donor when a kind runs dry.
    """
    spec = list(_cf.DEFAULT_CASEFILE_SPEC[:max(0, rounds)])
    while len(spec) < rounds:
        spec.append(_cf._FALLBACK_KIND)
    return tuple(spec)
...
def stash_allowance(rounds: int, ladder: Sequence[int]) -> int:
    ...
    IT IS ALSO THE RIGHT `hoard_allowance` FOR THE CASE. casefiles.py's default of 1
    was correct for a fixed three-finding casefile where every difficulty required
...
class ApfError(Exception):
    ...
    'the assignment failed' and 'this mystery cannot be assigned' need different
    responses from the caller -- the first can be re-run, the second cannot."""
...
def open_session(mystery: dict, players: Sequence[dict],
                 ...
                 seeds: int = _cf.DEFAULT_SEED_SEARCH) -> dict:
    """Build every player's casefile and set the rhythm going. Returns a plain,
    ...
    NOTHING IS FACE UP YET. The session opens at round 0 with every casefile assigned
    but unrevealed; `open_round()` turns one finding per player up. Building every
    casefile at open and revealing it progressively (rather than choosing a
    finding per round as the round arrives) is what keeps casefiles.py's guarantees
    ...
    spec = casefile_spec_for(rounds)
    allowance = stash_allowance(rounds, ladder)

    result = _cf.best_assignment(mystery, player_count, seeds=seeds,
                             casefile_spec=spec, hoard_allowance=allowance)
    if not result.ok:
        raise ApfError("no legal assignment for this mystery", result.issues)
...
def assigned(session: dict, player_id: str) -> List[dict]:
    """The findings this player has actually been handed so far: the first
    `round` slots of their casefile. Everything past that is assigned but face down,
    and no client is ever sent it."""
    return list(session["casefiles"].get(player_id, []))[:session["round"]]


def held_count(session: dict, player_id: str) -> int:
    return len(assigned(session, player_id))
...
def checkpoint_met(session: dict) -> bool:
    ...
    next round may be assigned. Also true at rounds with no checkpoint."""
    return all(outstanding(session, pid) == 0 for pid in session["player_order"])


def open_round(session: dict) -> int:
    ...
    a round that arrives before the table has paid is just a faster reveal.
    """
    if session["disclosed"]:
        raise ApfError("the case is closed; full disclosure has already run")
    if session["round"] >= session["rounds"]:
        raise ApfError(f"all {session['rounds']} rounds have been assigned")
...
def share(session: dict, player_id: str, finding_ids: Sequence[str]) -> dict:
    ...
    Validates against what the player has actually been ASSIGNED, not their whole
    casefile: a player cannot share round 4's finding during round 2, because they
    ...
    face_up = {f["id"] for f in assigned(session, player_id)}
    wanted = [str(fid) for fid in finding_ids]
    unknown = [fid for fid in wanted if fid not in face_up]
    if unknown:
        raise ApfError(f"not in this player's assigned findings: {unknown}")
...
def disclose(session: dict) -> List[dict]:
    ...
    for pid in session["player_order"]:
        # Everything ASSIGNED, not the whole casefile: findings past the last round
        # were never in anybody's possession, so nobody withheld them.
        for finding in assigned(session, pid):
...
def _finding_index(session: dict) -> Dict[str, dict]:
    """finding id -> {**finding, holder_id, holder_name}. Every assigned finding
    has exactly one holder, which is what lets the pool and the board attribute
    without a second lookup."""
    index: Dict[str, dict] = {}
    for pid in session["player_order"]:
        for finding in session["casefiles"].get(pid, []):
            index[finding["id"]] = {
                **finding,
                "holder_id": pid,
...
def shared_pool(session: dict) -> List[dict]:
    ...
    casefile, because the ORDER is the social record."""
...
def board(session: dict, mystery: dict) -> List[dict]:
    ...
    """
    ev_by_id = _cf.evidence_by_id(mystery)
    rows = {name: {"name": name, "cleared": False, "cleared_by": []}
            for name in _cf.suspects(mystery)}

    for entry in shared_pool(session):
        finding = _cf.Finding(id=entry["id"], kind=entry["kind"],
                                title=entry["title"], body=entry["body"],
                                reveals=list(entry.get("reveals") or []))
        for name in sorted(_cf.exonerated_by([finding], ev_by_id)):
            row = rows.get(name)
            if row is None:
    ...
    return [rows[name] for name in _cf.suspects(mystery)]
...
def can_prove(session: dict, player_id: str, mystery: dict) -> bool:
    ...
    their own casefile. YOU ALWAYS KNOW WHAT YOU KEPT -- the same premise
    casefiles.proof_survives_hoarding() enumerates over, applied to the live game.
    ...
    for finding in assigned(session, player_id):
        known.setdefault(finding["id"], finding)
    findings = [_cf.Finding(id=f["id"], kind=f["kind"], title=f["title"],
                              body=f["body"], reveals=list(f.get("reveals") or []))
                for f in known.values()]
    return _cf.solves(findings, mystery, ev_by_id)
...
def state_for(session: dict, player_id: str, mystery: dict) -> dict:
    ...
    casefile = [{**f, "shared": f["id"] in my_shared} for f in assigned(session, player_id)]
