#!/usr/bin/env python3
"""
test_apf.py -- the rhythm: dealt over rounds, shared at a cumulative checkpoint.
...
implementation could get wrong in a way no other checker would notice:
  ...
  - round 1 has NO checkpoint (the minimum's floor of 1 makes a hand of one a
    tax rather than a decision)
  - a shared finding STAYS in its holder's hand, and sharing is monotone
  ...
  - the difficulty ladder SEPARATES under the rhythm, which is the whole reason
    the rhythm exists -- Sessions 38 and 39 both measured it inert at a fixed hand
  ...
  - a client is never sent another player's hand, or its own undealt future
...
Fixtures, for the reason scripts/test_deal.py gives -- plus one pass over the
...
def pay_up(s):
    ...
    have not already shared. Slicing from the front of the hand does NOT work:
    ...
    for pid in s["player_order"]:
        ...
        fresh = [f["id"] for f in apf.dealt(s, pid) if f["id"] not in already]
        apf.share(s, pid, fresh[:need])


def play_out(s, stop_short=False):
    """Deal every round, paying each checkpoint with the bare minimum. Leaves
...
def test_round_count_and_spec():
    print("\nRounds = findings / players, and the hand spec is that long")
    ...
    spec = apf.hand_spec_for(4)
    check("the spec's LENGTH is the round count, so deal.py needs no new code",
          len(spec) == 4, str(spec))
    check("it keeps APF's specified hand and extends with the fallback kind",
          spec[:3] == D.DEFAULT_HAND_SPEC and spec[3] == D._FALLBACK_KIND, str(spec))
    check("a short session truncates rather than padding",
          apf.hand_spec_for(2) == D.DEFAULT_HAND_SPEC[:2], str(apf.hand_spec_for(2)))


def test_the_ladder_separates():
    """The reason the rhythm exists. Sessions 38 and 39 both measured the
    difficulty ladder inert at a fixed three-finding hand -- all three
    ...
    """
    print("\nThe difficulty ladder, which a fixed hand had killed")
    stash = lambda r, rl: [n - v for n, v in zip(range(1, r + 1), apf.share_ladder(r, rl))]

    check("at a fixed hand of three the three difficulties are identical",
...
def test_round_one_has_no_decision():
    print("\nRound 1 deals and does not ask -- the floor of 1 makes it a tax")
    m, s = open4()
    ...
    apf.deal_round(s)
    check("round 1 deals one finding each", apf.held_count(s, "p1") == 1)
    check("round 1 asks for nothing", apf.required_now(s, "p1") == 0)
    check("so no checkpoint is open", not apf.checkpoint_open(s))
    check("and round 2 may be dealt without anyone sharing", apf.deal_round(s) == 2)
...
def test_sharing_is_cumulative_and_monotone():
    print("\nA shared finding stays in the hand; the requirement is cumulative")
    m, s = open4(MEDIUM)
    apf.deal_round(s)
    apf.deal_round(s)
    ...
    check("the finding is still in the holder's hand after sharing",
          first in [f["id"] for f in apf.dealt(s, "p1")])
    ...
    for pid in ("p2", "p3", "p4"):
        apf.share(s, pid, [apf.dealt(s, pid)[0]["id"]])
    check("once everyone has paid, the round advances", apf.deal_round(s) == 3)
    ...
    for _ in range(3):
        if apf.checkpoint_open(s2):
            for pid in s2["player_order"]:
                ...
                fresh = [f["id"] for f in apf.dealt(s2, pid) if f["id"] not in already]
    ...
    try:
        ...
        check("sharing a finding you were never dealt is refused", False)
    except apf.ApfError:
        check("sharing a finding you were never dealt is refused", True)

    # A future round's finding is dealt but face down: it cannot be spent early.
    m3, s3 = open4(MEDIUM)
    apf.deal_round(s3)
    future = s3["hands"]["p1"][3]["id"]
    try:
...
def test_the_round_cannot_outrun_the_checkpoint():
    ...
    try:
        apf.deal_round(s)
        check("dealing over an open checkpoint is refused", False)
    except apf.ApfError as e:
        check("dealing over an open checkpoint is refused", True)
...
def test_the_board():
    ...
    # Share any dealt finding that clears somebody, and watch the face grey out.
    ev = D.evidence_by_id(m)
    as_finding = lambda f: D.Finding(id=f["id"], kind=f["kind"], title=f["title"],
                                     body=f["body"], reveals=list(f["reveals"]))
    holder, fid, cleared_names = None, None, []
    for pid in s["player_order"]:
        for f in apf.dealt(s, pid):
            names = sorted(D.exonerated_by([as_finding(f)], ev))
            if names:
                holder, fid, cleared_names = pid, f["id"], names
                break
        if holder:
            break
    check("round 1 dealt somebody a finding that clears a suspect",
          holder is not None, "no exonerating finding in any hand at round 1")
    ...
    # carrier per suspect is what makes proof die under hoarding (deal.py's own
    # measurement), and a fixture that cannot be dealt proves nothing about the
    # board.
    glove = mystery(
        [("E1", ["Ortiz"], []), ("E2", ["Ortiz"], []),
         ("E3", ["Brand"], []), ("E4", ["Brand"], []),
    ...
    for pid in gs["player_order"]:
        apf.share(gs, pid, [f["id"] for f in apf.dealt(gs, pid)])
...
def test_full_disclosure():
    ...
    withheld_before = sum(len(apf.dealt(s, pid)) - len(s["shared"][pid])
                          for pid in s["player_order"])
    ...
    check("the pool now holds every dealt finding",
          len(apf.shared_pool(s)) == sum(len(apf.dealt(s, pid))
                                         for pid in s["player_order"]))
    try:
        apf.share(s, "p1", [apf.dealt(s, "p1")[0]["id"]])
...
def test_a_client_is_sent_only_its_own_half():
    ...
    check("the hand is this player's dealt findings only",
          [f["id"] for f in st["hand"]] == [f["id"] for f in apf.dealt(s, "p1")])
    check("it stops at the current round -- no future findings",
          len(st["hand"]) == 2 and s["rounds"] == 4)
    ...
    others = {f["id"] for pid in ("p2", "p3", "p4") for f in s["hands"][pid]
    ...
    future = {f["id"] for f in s["hands"]["p1"][2:]}
...
def test_refusals():
    ...
    # An undealable MYSTERY is a different refusal from an unlucky deal.
    ...
    try:
        ...
        check("and it carries deal.py's own diagnosis rather than a bare message",
...
def test_against_the_accepted_mystery():
    ...
    for label, share_rule in (("EASY", EASY), ("MEDIUM", MEDIUM), ("HARD", HARD)):
        ...
        check(f"{label}: the dealing has no monopoly on proof",
              s["deal"]["monopoly"] == 0, str(s["deal"]))
        play_out(s)
        held_back = sum(len(apf.dealt(s, pid)) - len(s["shared"][pid])
                        for pid in s["player_order"])
        ...
        check(f"{label}: full disclosure leaves exactly the culprit standing",
              standing == [D.culprit(m)], str(standing))
